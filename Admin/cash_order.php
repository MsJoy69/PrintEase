...
// Ensure necessary columns exist (including created_at for month filtering)
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS order_id VARCHAR(50) NOT NULL AFTER id");
...
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_amount DECIMAL(10,2) NULL"); 
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_proof VARCHAR(255) NULL"); 
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_reference VARCHAR(100) NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS created_at DATETIME DEFAULT CURRENT_TIMESTAMP"); // ENSURE THIS EXISTS
...
if (isset($_POST['update_status'])) {
    try {
        $pk_id = intval($_POST['order_id']); // Internal Primary Key
        $new_status = $_POST['status'];

        $update = $conn->prepare("UPDATE payments SET status = ?, status_time = NOW() WHERE id = ?");
        $update->bind_param("si", $new_status, $pk_id);
        $update->execute();

        // Fetch the custom order_id string alongside user info
        ...
        if (empty($orderInfo) || empty($orderInfo['user_id'])) {
            // Note: Cash orders typically don't have a user_id, but keeping logic consistent
            // If user_id is null, skip notification
        } else {
            $user_id = (int)$orderInfo['user_id'];
            $customer_name = $orderInfo['customer_name'] ?? 'User';
            $display_order_id = $orderInfo['order_id'] ?? $pk_id; 
            
            // Use $display_order_id in messages
            switch ($new_status) {
                case 'Pending':
                    $message = "Hello $customer_name, your cash order #$display_order_id is now Pending. Please wait for processing.";
                    break;
                case 'Processing':
                    $message = "Your cash order #$display_order_id is now Processing. We'll notify you when it's completed.";
                    break;
                case 'Completed':
                    $message = "Your cash order #$display_order_id has been Completed. Thank you!";
                    break;
                case 'Cancelled':
                    $message = "Your cash order #$display_order_id has been Cancelled. Contact support if needed.";
                    break;
                case 'Received': 
                    $message = "🎉 Great News! Your cash order #$display_order_id has been successfully Received. Thank you for choosing us!";
                    break;
                default:
                    $message = "Your cash order #$display_order_id status changed to $new_status.";
            }
        ...
        header("Location: cash_order.php?updated=1");
        exit();

    } catch (Exception $e) {
        // Output error for debugging, then exit
        echo "<div style='color:red;padding:20px;'>Notification error: " . htmlspecialchars($e->getMessage()) . "</div>";
        exit();
    }
}

// Search, Filter and Month LOGIC
...
// UPDATED QUERY:
// 1. Checks for 'Cash' (case insensitive via implicit MySQL string comparison or you can add LOWER())
// 2. Checks if 'downpayment_proof' exists (syncs with manage_order.php logic for CASH links)
$sql = "SELECT * FROM payments WHERE (
    order_type = 'Cash' 
    OR payment_proof = 'Cash' 
    OR (downpayment_proof IS NOT NULL AND downpayment_proof != '')
)";
...
if (!empty(trim($search))) {
    $conditions[] = "(order_id LIKE ? OR id LIKE ? OR customer_name LIKE ? OR reference_number LIKE ? OR payment_proof LIKE ?)";
...
// COUNT ORDERS - Updated logic for counts as well
$countSqlBase = "SELECT COUNT(*) AS c FROM payments WHERE (order_type = 'Cash' OR payment_proof = 'Cash' OR (downpayment_proof IS NOT NULL AND downpayment_proof != ''))";

$totalOrders = $conn->query($countSqlBase)->fetch_assoc()['c'];
$pendingCount = $conn->query($countSqlBase . " AND status='Pending'")->fetch_assoc()['c'];
$processingCount = $conn->query($countSqlBase . " AND status='Processing'")->fetch_assoc()['c'];
$completedCount = $conn->query($countSqlBase . " AND status = 'Completed'")->fetch_assoc()['c'];
$receivedCount = $conn->query($countSqlBase . " AND status='Received'")->fetch_assoc()['c'];
$cancelledCount = $conn->query($countSqlBase . " AND status='Cancelled'")->fetch_assoc()['c'];
...
<!-- Assuming the CSS is external, but adding the new styles internally -->
<link rel="stylesheet" href="online_order.css?v=2"> 

<style>
     @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap');
    /* Styles for the Design File section in Modal */
    .design-file-header {
        display: flex;
    ...
    /* Control the width of the search input group */
    .search-input-group {
        flex-grow: 1; 
        max-width: 450px; /* Limit the maximum width of the search bar */
    }
...
<div class="sidebar" id="sidebar">
    ...
    <a href="cash_order.php" class="active"><i class="fas fa-money-bill"></i> Cash Order</a>
...
<div class="content">
    ...
    <div class="header-top-row">
        <h2 class="mb-0">Cash Orders</h2>
        <div class="current-datetime" id="currentDateTime"></div>
    </div>
    
    <div class="row mb-4 text-center">
        <!-- Card counts are specific to Cash Orders -->
        <div class="col mb-3">
            <div class="card bg-c-green" onclick="window.location='cash_order.php?filter=Pending&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
        ...
        <div class="col mb-3">
            <div class="card bg-c-yellow" onclick="window.location='cash_order.php?filter=Processing&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
        ...
        <div class="col mb-3">
            <div class="card bg-success" onclick="window.location='cash_order.php?filter=Completed&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6 style="color:white;">Orders Completed</h6>
                    <h2 class="fw-bold mb-0"><?= $completedCount ?></h2>
                </div>
            </div>
        </div>
        <div class="col mb-3">
            <div class="card bg-danger text-white" onclick="window.location='cash_order.php?filter=Received&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6>Orders Received</h6>
                    <h2 class="fw-bold mb-0"><?= $receivedCount ?></h2>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="table-container mb-4">
        <!-- START NEW FILTER FORM ROW -->
        <form method="GET" action="cash_order.php" class="filter-form-row">
            ...
            <?php if (!empty($search) || $monthFilter !== 'All' || $processFilter !== 'All'): ?>
                <small class="text-muted mt-2 d-block w-100">
                    ...
                    <?php endif; ?>
                    . 
                    <a href="cash_order.php?filter=<?= urlencode($filter) ?>" class="text-danger">Clear All Filters</a>
                </small>
            <?php endif; ?>
        </form>
        
        <!-- END NEW FILTER FORM ROW -->
    </div>
        ...
        <table class="table table-bordered table-hover text-center align-middle">
            ...
            <?php if($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): 
                    // Cash orders primarily use downpayment_proof or no proof at all
                    $imgUrl = resolve_image_path($row['downpayment_proof']);
                ...
                ?>
                <tr>
                    <td class="fw-bold text-primary"><?= htmlspecialchars($displayOrderId) ?></td>
                    ...
                    <td>
                        <a href="#" class="customer-link" 
                            ...
                            data-downpayment-amount="<?= $row['downpayment_amount'] ?? 0 ?>"
                    ...
                    <td>
                        <?php if(!empty($imgUrl)): ?>
                             <a href="#" class="cash-downpayment-link"
                                data-bs-toggle="modal"
                                data-bs-target="#downpaymentModal"
                                data-img="<?= htmlspecialchars($imgUrl) ?>"
                                data-total="<?= number_format($row['total_price'], 2) ?>"
                                data-downpayment="<?= number_format($row['downpayment_amount'], 2) ?>"
                                data-remaining="<?= number_format($row['total_price'] - $row['downpayment_amount'], 2) ?>"
                                data-reference="<?= htmlspecialchars($row['downpayment_reference'] ?? 'N/A') ?>">
                                Downpayment
                            </a>
                        <?php else: ?>
                    ...
                    <td>
                        <span class="badge 
                            <?= ($row['status']=='Pending'?'bg-warning':($row['status']=='Processing'?'bg-info':($row['status']=='Completed'?'bg-success':($row['status']=='Received'?'bg-danger':'bg-danger')))) ?>">
                    ...
                    <td>
                        <form method="POST" class="d-flex justify-content-center align-items-center">
                            ...
                            <?php 
                                ...
                                $isFinalState = ($currentStatus == 'Completed' || $currentStatus == 'Received' || $currentStatus == 'Cancelled');
                            ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="9" class="text-muted">No cash orders found with the current search and filter criteria.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
...
<!-- Proof Modal (for full payment proof, though less common in cash orders) -->
...
<!-- Downpayment Modal (for cash downpayment proof) -->
<div class="modal fade" id="downpaymentModal" tabindex="-1" aria-labelledby="downpaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="downpaymentModalLabel">Downpayment Details</h5>
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <table class="payment-breakdown-table">
              ...
              <tr>
                <td>Downpayment:</td>
                <td id="modalDownpaymentAmount">₱0.00</td>
              </tr>
              <tr>
                <td>Remaining Amount:</td>
                <td id="modalRemainingAmount">₱0.00</td>
              </tr>
            </table>
            <p class="mt-3"><strong>Reference Number:</strong> <span id="modalDownpaymentReference">N/A</span></p>
          </div>
          <div class="col-md-6">
            <h6><strong>Proof of Downpayment</strong></h6>
            <img src="" id="downpaymentProofImage" class="modal-img" alt="Downpayment Proof">
          </div>
        </div>
      </div>
...
<div class="modal fade" id="orderDetailsModal" tabindex="-1" aria-labelledby="orderDetailsModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="orderDetailsModalLabel">Order <span id="modalOrderIdHeader" class="fw-bold"></span></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            ...
            <p><strong>Customer Name:</strong> <span id="modalCustomerNameBody"></span></p>
          ...
          <div class="col-md-6">
            ...
            <div class="design-file-header">
                <h6><strong>Design File</strong></h6>
                <div id="downloadAllBtnContainer"></div>
            </div>
            <div id="modalDesignFileContainer"></div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
if (downpaymentModal) {
    downpaymentModal.addEventListener('show.bs.modal', event => {
        const button = event.relatedTarget;
        ...
        document.getElementById('downpaymentProofImage').src = imgSrc;
        // Safe fallback for elements
        if(document.getElementById('modalTotalAmount')) document.getElementById('modalTotalAmount').textContent = '₱' + total;
        if(document.getElementById('modalDownpaymentAmount')) document.getElementById('modalDownpaymentAmount').textContent = '₱' + downpayment;
        if(document.getElementById('modalRemainingAmount')) document.getElementById('modalRemainingAmount').textContent = '₱' + remaining;
        if(document.getElementById('modalDownpaymentReference')) document.getElementById('modalDownpaymentReference').textContent = reference;
    });
}
...
if (orderDetailsModal) {
    orderDetailsModal.addEventListener('show.bs.modal', event => {
        ...
        const safeSetText = (selector, text) => {
            const el = orderDetailsModal.querySelector(selector);
            if (el) {
                el.textContent = text;
            } else {
        ...
        // Set Values Safely
        safeSetText('#modalOrderIdHeader', '#' + orderId);
        safeSetText('#modalOrderId', orderId);
        safeSetText('#modalCustomerNameBody', customerName);
        safeSetText('#modalEmail', email);
        safeSetText('#modalContact', contact);
        safeSetText('#modalAddress', address);
        safeSetText('#modalOrderType', orderType);
        safeSetText('#modalQuantity', quantity);
        safeSetText('#modalExpectedDate', expectedDate);
        ...
        if (downpaymentAmount > 0) {
            if(paymentBreakdownDiv) paymentBreakdownDiv.style.display = 'block';
            if(simpleTotalPriceP) simpleTotalPriceP.style.display = 'none';
            
            // Global replacement for commas to handle thousands (e.g., 1,500.00)
            const totalValue = parseFloat(totalPrice.replace('₱', '').replace(/,/g, ''));
            const remaining = totalValue - downpaymentAmount;
            
            safeSetText('#modalDetailTotalPrice', totalPrice);
            safeSetText('#modalDetailDownpayment', '₱' + downpaymentAmount.toFixed(2));
            safeSetText('#modalDetailRemaining', '₱' + remaining.toFixed(2));
        } else {
            if(paymentBreakdownDiv) paymentBreakdownDiv.style.display = 'none';
            if(simpleTotalPriceP) simpleTotalPriceP.style.display = 'block';
            safeSetText('#modalTotalPriceValue', totalPrice);
        }
        ...
        const downloadAllContainer = orderDetailsModal.querySelector('#downloadAllBtnContainer');
        
        if (modalDesignFileContainer) modalDesignFileContainer.innerHTML = ''; 
        if (downloadAllContainer) downloadAllContainer.innerHTML = '';
        ...
        if (designFile) {
            ...
            files.forEach((cleanedFile, index) => {
                if (cleanedFile) {
                    const fileUrl = '../uploads/designs/' + cleanedFile;
                    fileLinks.push({ url: fileUrl, name: cleanedFile }); 

                    const downloadLink = document.createElement('a');
                    downloadLink.href = fileUrl;
                    downloadLink.textContent = `Design File ${index + 1}`;
                    downloadLink.className = 'btn btn-primary btn-sm me-1 mb-1';
                    downloadLink.setAttribute('download', cleanedFile);
                    if(modalDesignFileContainer) modalDesignFileContainer.appendChild(downloadLink);
                }
            });

            if (files.length > 1 && downloadAllContainer) {
                const downloadAllBtn = document.createElement('button');
                downloadAllBtn.textContent = 'Download All (.zip)';
                downloadAllBtn.className = 'btn btn-outline-success btn-sm download-all-btn';
                ...
                downloadAllBtn.onclick = async () => {
                    ...
                    try {
                        ...
                        zip.generateAsync({ type: "blob" })
                            .then(content => {
                                saveAs(content, `Order_${orderId}_Designs.zip`);
                                downloadAllBtn.textContent = 'Download All (.zip)';
            ...
            if(modalDesignFileContainer) modalDesignFileContainer.innerHTML = '<p class="text-muted">No design file was uploaded for this order.</p>';
        }
    });
}
</script>
