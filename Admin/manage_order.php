<?php
include 'auth.php';

$conn = new mysqli("localhost", "root", "", "payment");
if($conn->connect_error){
    die("Connection failed: " . $conn->connect_error);
}

$notifConn = new mysqli("localhost", "root", "", "notification");
if($notifConn->connect_error){
    die("Notification DB connection failed: " . $notifConn->connect_error);
}

// Ensure columns exist
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS order_id VARCHAR(50) AFTER id");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS reference_number VARCHAR(100) DEFAULT NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS payment_proof VARCHAR(255) DEFAULT NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'Pending'");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS design_file VARCHAR(255) DEFAULT NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS order_details TEXT NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_amount DECIMAL(10,2) NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_proof VARCHAR(255) NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS downpayment_reference VARCHAR(100) NULL");
$conn->query("ALTER TABLE payments ADD COLUMN IF NOT EXISTS created_at DATETIME DEFAULT CURRENT_TIMESTAMP"); // Ensure created_at exists for month filter

// Handle Status Update
ini_set('display_errors', 1);
error_reporting(E_ALL);
mysqli_report(MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT);

if (isset($_POST['update_status'])) {
    try {
        $pk_id = intval($_POST['order_id']); // Internal Primary Key
        $new_status = $_POST['status'];

        $update = $conn->prepare("UPDATE payments SET status = ? WHERE id = ?");
        $update->bind_param("si", $new_status, $pk_id);
        $update->execute();

        // Fetch the custom order_id string alongside user info
        $get = $conn->prepare("SELECT user_id, customer_name, order_id FROM payments WHERE id = ?");
        $get->bind_param("i", $pk_id);
        $get->execute();
        $res = $get->get_result();
        $orderInfo = $res->fetch_assoc();
        $get->close();

        if (empty($orderInfo) || empty($orderInfo['user_id'])) {
            header("Location: manage_order.php?updated=0&msg=no_user");
            exit();
        }

        $user_id = (int)$orderInfo['user_id'];
        $customer_name = $orderInfo['customer_name'] ?? 'User';
        $display_order_id = $orderInfo['order_id'] ?? $pk_id; // Use custom ID, fallback to PK if missing
        
        // Use $display_order_id in messages
        switch ($new_status) {
            case 'Pending':
                $message = "Hello $customer_name, your order #$display_order_id is now Pending. Please wait for processing.";
                break;
            case 'Processing':
                $message = "Your order #$display_order_id is now Processing. We'll notify you when it's completed.";
                break;
            case 'Completed':
                $message = "Your order #$display_order_id has been Completed. Thank you!";
                break;
            case 'Cancelled':
                $message = "Your order #$display_order_id has been Cancelled. Contact support if needed.";
                break;
            case 'Received': 
                $message = "🎉 Great News! Your order #$display_order_id has been successfully Received. Thank you for choosing us!";
                break;
            default:
                $message = "Your order #$display_order_id status changed to $new_status.";
        }

        $notifStmt = $notifConn->prepare("INSERT INTO notifications (user_id, order_id, message, status, created_at) VALUES (?, ?, ?, 'unread', NOW())");
        $notifStmt->bind_param("iis", $user_id, $pk_id, $message);
        $notifStmt->execute();
        $notifStmt->close();

        header("Location: manage_order.php?updated=1");
        exit();

    } catch (Exception $e) {
        // Output error for debugging, then exit
        echo "<div style='color:red;padding:20px;'>Notification error: " . htmlspecialchars($e->getMessage()) . "</div>";
        exit();
    }
}

// Search, Filter and Month LOGIC
$filter = $_GET['filter'] ?? 'All';
$search = $_GET['search'] ?? '';
// NEW: Month selection - must be two digits (e.g., 01, 12) or 'All'
$monthFilter = $_GET['month'] ?? 'All'; 
$processFilter = $_GET['process'] ?? 'All';

$searchParam = "%" . trim($search) . "%";

$sql = "SELECT * FROM payments";
$conditions = [];
$types = "";
$params = [];

// 1. Status Filter
if($filter !== 'All'){
    if ($filter === 'Completed') {
        $conditions[] = "status = 'Completed'";
    } elseif ($filter === 'Received') {
        $conditions[] = "status = 'Received'";
    } else {
        $conditions[] = "status = ?";
        $types .= "s";
        $params[] = $filter;
    }
}

// 2. Month Filter (NEW)
if ($monthFilter !== 'All' && !empty($monthFilter)) {
    if ($monthFilter === 'today') {
        $conditions[] = "DATE(created_at) = CURDATE()";
    } elseif ($monthFilter === 'week') {
        $conditions[] = "YEARWEEK(created_at, 1) = YEARWEEK(CURDATE(), 1)";
    } elseif ($monthFilter === 'year') { 
        // NEW: Filter for This Year
        $conditions[] = "YEAR(created_at) = YEAR(CURDATE())";
    } elseif (is_numeric($monthFilter)) {
        $conditions[] = "MONTH(created_at) = ?";
        $types .= "i";
        $params[] = (int)$monthFilter;
    }
}

if ($processFilter !== 'All') {
    $conditions[] = "order_details LIKE ?";
    $types .= "s";
    // This will match "Rush Process" or "Standard Process" inside the JSON/Text
    $params[] = "%" . $processFilter . " Process%"; 
}


// 3. Search Term Filter
if (!empty(trim($search))) {
    $conditions[] = "(order_id LIKE ? OR id LIKE ? OR customer_name LIKE ? OR reference_number LIKE ? OR order_type LIKE ? OR payment_proof LIKE ?)";
    
    $types .= "ssssss"; 
    
    $params[] = $searchParam;
    $params[] = $searchParam;
    $params[] = $searchParam;
    $params[] = $searchParam;
    $params[] = $searchParam; 
    $params[] = $searchParam; 
}

if (!empty($conditions)) {
    $sql .= " WHERE " . implode(' AND ', $conditions);
}

$sql .= " ORDER BY id DESC";

if (!empty($params)) {
    $stmt = $conn->prepare($sql);
    if (!empty($params)) {
        $bind_params = array_merge([$types], $params);
        $references = [];
        foreach ($bind_params as $key => $value) {
            $references[$key] = &$bind_params[$key];
        }
        call_user_func_array([$stmt, 'bind_param'], $references);
    }
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $result = $conn->query($sql);
}

// COUNT ORDERS - Note: Counts still display ALL orders, not filtered ones, for consistency with dashboard cards.
$totalOrders = $conn->query("SELECT COUNT(*) AS c FROM payments")->fetch_assoc()['c'];
$pendingCount = $conn->query("SELECT COUNT(*) AS c FROM payments WHERE status='Pending'")->fetch_assoc()['c'];
$processingCount = $conn->query("SELECT COUNT(*) AS c FROM payments WHERE status='Processing'")->fetch_assoc()['c'];
$completedCount = $conn->query("SELECT COUNT(*) AS c FROM payments WHERE status = 'Completed'")->fetch_assoc()['c'];
$receivedCount = $conn->query("SELECT COUNT(*) AS c FROM payments WHERE status='Received'")->fetch_assoc()['c'];
$cancelledCount = $conn->query("SELECT COUNT(*) AS c FROM payments WHERE status='Cancelled'")->fetch_assoc()['c'];

function resolve_image_path($storedPath) {
    if (empty($storedPath)) return null;
    $storedPath = str_replace('\\', '/', $storedPath);
    $candidates = [
        $storedPath,
        'components/' . $storedPath,
        '../' . $storedPath,
        '../components/' . $storedPath,
        'uploads/' . basename($storedPath),
        'components/uploads/' . basename($storedPath),
        '../uploads/payments/' . basename($storedPath),
        'uploads/payments/' . basename($storedPath)
    ];
    foreach ($candidates as $c) {
        $fsPath = __DIR__ . '/' . $c;
        if (file_exists($fsPath)) {
            return $c;
        }
    }
    if (strpos($storedPath, '/') === 0) {
        $docRootPath = $_SERVER['DOCUMENT_ROOT'] . $storedPath;
        if (file_exists($docRootPath)) return $storedPath;
    }
    return null;
}

// Array of Months for Dropdown
$months = [
    1 => 'January',
    2 => 'February',
    3 => 'March',
    4 => 'April',
    5 => 'May',
    6 => 'June',
    7 => 'July',
    8 => 'August',
    9 => 'September',
    10 => 'October',
    11 => 'November',
    12 => 'December'
];

?>

<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Manage Orders</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
<link rel="stylesheet" href="manage_order.css">

<style>
     @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap');
    .sidebar a:hover,
.sidebar a.active {
    background-color: #198754;
    color: white;
}
    /* Styles for the Design File section in Modal */
    .design-file-header {
        display: flex;
        justify-content: space-between;
        align-items: center;
        margin-bottom: 10px;
    }
    .download-all-btn {
        font-size: 0.8rem;
        padding: 4px 8px;
    }
    /* Ensure modal content is readable */
    .modal-body p {
        margin-bottom: 8px;
    }
    .payment-breakdown-table {
        width: 100%;
        margin-top: 10px;
        font-size: 0.95rem;
    }
    .payment-breakdown-table td {
        padding: 3px 0;
    }
    .payment-breakdown-table td:last-child {
        text-align: right;
        font-weight: bold;
    }
    /* NEW CSS for Month Filter and Search Layout */
    .filter-form-row {
        display: flex;
        flex-wrap: wrap; /* Allows wrapping on smaller screens */
        gap: 15px; /* Space between filter elements */
        align-items: center;
        margin-bottom: 20px;
    }
    .month-filter-group {
        display: flex;
        align-items: center;
        gap: 8px;
        flex-shrink: 0; /* Prevents it from shrinking too much */
    }
    .search-input-group {
        flex-grow: 1; /* Allows the search bar to take up remaining space */
        max-width: 450px; /* Limit the maximum width of the search bar */
    }
    .month-filter-group label {
        font-weight: 500;
        color: #333;
    }

    .logout-btn {
    /* The critical line: pushes the element to the bottom */
    margin-top: auto; 
    /* Set specific styling for the button, adjust width/margin as needed */
    width: 90%; /* Adjust width to fit the sidebar */
    margin: 200px auto 20px auto; /* Center it with more space at the bottom */
    text-align: center;
    border-radius: 5px;
    padding: 10px;
    box-sizing: border-box; /* Include padding in width calculation */
}
    /* Style for the month dropdown */
    .month-filter-group .form-select {
        width: 150px; /* Set a fixed width for the month dropdown */
        padding: 8px 12px;
        border-radius: 6px;
        border: 1px solid #ced4da;
        background-color: white;
        font-size: 1rem;
        color: #495057;
    }
    @media (max-width: 768px) {
        .filter-form-row {
            flex-direction: column;
            align-items: flex-start;
        }
        .search-input-group {
            width: 100%;
            max-width: none;
        }
    }
</style>
</head>
<body>

<button class="burger-icon" id="sidebarToggle">
    <i class="fas fa-bars"></i>
</button>

<div class="sidebar" id="sidebar">
    <h4>PrintEase</h4>
    <a href="index.php"><i class="fas fa-home"></i> Dashboard</a>
    <a href="customer_chat.php"><i class="fa-solid fa-message"></i> Customer Chat</a>
    <a href="manage_order.php" class="active"><i class="fas fa-tasks"></i> Manage Orders</a>
    <a href="customer.php"><i class="fas fa-users"></i> Customer Management</a>
    <a href="online_order.php"><i class="fas fa-credit-card"></i> Online Order</a>
    <a href="cash_order.php"><i class="fas fa-money-bill"></i> Cash Order</a>
    <a href="received_order.php"><i class="fas fa-money-bill"></i> Received Order</a>
    <a href="admin_profile.php"><i class="fas fa-user-circle"></i> Admin Profile</a>

    <a href="logout.php" class="logout-btn"><i class="fas fa-sign-out-alt"></i> LOGOUT</a>
</div>

<div class="overlay" id="overlay"></div>

<div class="content">
    
    <div class="header-top-row">
        <h2 class="mb-0">Manage Orders</h2>
        <div class="current-datetime" id="currentDateTime"></div>
    </div>
    
    <div class="row mb-4 text-center">
        <div class="col mb-3">
            <div class="card bg-c-green" onclick="window.location='manage_order.php?filter=Pending&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6>Orders Pending</h6>
                    <h2 class="fw-bold mb-0"><?= $pendingCount ?></h2>
                </div>
            </div>
        </div>
        <div class="col mb-3">
            <div class="card bg-c-yellow" onclick="window.location='manage_order.php?filter=Processing&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6>Orders Processing</h6>
                    <h2 class="fw-bold mb-0"><?= $processingCount ?></h2>
                </div>
            </div>
        </div>
        <div class="col mb-3">
            <div class="card bg-success" onclick="window.location='manage_order.php?filter=Completed&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6 style="color:white;">Orders Completed</h6>
                    <h2 class="fw-bold mb-0"><?= $completedCount ?></h2>
                </div>
            </div>
        </div>
        <div class="col mb-3">
            <div class="card bg-danger text-white" onclick="window.location='manage_order.php?filter=Received&month=<?= urlencode($monthFilter) ?>&search=<?= urlencode($search) ?>'">
                <div class="card-body d-flex flex-column justify-content-center">
                    <h6>Orders Received</h6>
                    <h2 class="fw-bold mb-0"><?= $receivedCount ?></h2>
                </div>
            </div>
        </div>
    </div>

    <?php if(isset($_GET['updated'])): ?>
        <div class="alert alert-success text-center">Order status updated successfully!</div>
    <?php endif; ?>
    
    <div class="table-container mb-4">
        <!-- START NEW FILTER FORM ROW -->
            <form method="GET" action="manage_order.php" class="filter-form-row">
            <input type="hidden" name="filter" value="<?= htmlspecialchars($filter) ?>">
            
            <div class="month-filter-group">
                <label for="month-select" class="mb-0">Date:</label>
                <select id="month-select" name="month" class="form-select" onchange="this.form.submit()">
                    <option value="All" <?= $monthFilter == 'All' ? 'selected' : '' ?>>All Time</option>
                    <option value="today" <?= $monthFilter == 'today' ? 'selected' : '' ?>>Today</option>
                    <option value="week" <?= $monthFilter == 'week' ? 'selected' : '' ?>>This Week</option>
                    <option value="year" <?= $monthFilter == 'year' ? 'selected' : '' ?>>This Year</option>
                    <option disabled>──────────</option>
                    <?php foreach ($months as $num => $name): ?>
                        <option value="<?= $num ?>" <?= $monthFilter == $num ? 'selected' : '' ?>>
                            <?= $name ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>

            <div class="month-filter-group">
                <label for="process-select" class="mb-0">Process:</label>
                <select id="process-select" name="process" class="form-select" style="width: 140px;" onchange="this.form.submit()">
                    <option value="All" <?= $processFilter == 'All' ? 'selected' : '' ?>>All Types</option>
                    <option value="Standard" <?= $processFilter == 'Standard' ? 'selected' : '' ?>>Standard</option>
                    <option value="Rush" <?= $processFilter == 'Rush' ? 'selected' : '' ?>>Rush</option>
                </select>
            </div>
            
            <div class="input-group search-input-group">
                <input type="text" class="form-control" name="search" 
                    placeholder="Search Order ID, Name..." 
                    value="<?= htmlspecialchars($search) ?>">
                <button class="btn btn-primary" type="submit">🔍 Search</button>
            </div>
            
            <?php if (!empty($search) || $monthFilter !== 'All' || $processFilter !== 'All'): ?>
                <small class="text-muted mt-2 d-block w-100">
                    Showing results for 
                    <?php if ($processFilter !== 'All'): ?>
                        <strong><?= htmlspecialchars($processFilter) ?> Process</strong> orders
                    <?php endif; ?>
                    <?php if ($monthFilter !== 'All'): ?>
                        in <strong><?= htmlspecialchars($months[(int)$monthFilter] ?? $monthFilter) ?></strong>
                    <?php endif; ?>
                    <?php if (!empty($search)): ?>
                        matching "<strong><?= htmlspecialchars($search) ?></strong>"
                    <?php endif; ?>
                    . 
                    <a href="manage_order.php?filter=<?= urlencode($filter) ?>" class="text-danger">Clear All Filters</a>
                </small>
            <?php endif; ?>
        </form>
        <!-- END NEW FILTER FORM ROW -->
    </div>

    <div class="table-container">
        <table class="table table-bordered table-hover text-center align-middle">
            <thead>
                <tr>
                    <th>Order ID</th>
                    <th>Reference No.</th>
                    <th>Customer</th>
                    <th>Order Type</th>
                    <th>Expected Date</th>
                    <th>Total Price</th>
                    <th>Payment Proof</th>
                    <th>Status</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
            <?php if($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): 
                    $imgUrl = resolve_image_path($row['payment_proof']);
                    $downpaymentImgUrl = resolve_image_path($row['downpayment_proof']);
                    
                    $expectedDate = $row['expected_date'];
                    $displayDate = 'N/A';
                    if (!empty($expectedDate) && $expectedDate != '0000-00-00') {
                        try {
                            $dateTime = new DateTime($expectedDate);
                            $displayDate = $dateTime->format('M d, Y');
                        } catch (Exception $e) {
                            $displayDate = htmlspecialchars($expectedDate);
                        }
                    }
                    
                    // Fallback if order_id is missing (for old records)
                    $displayOrderId = !empty($row['order_id']) ? $row['order_id'] : $row['id'];
                ?>
                <tr>
                    <td class="fw-bold text-primary"><?= htmlspecialchars($displayOrderId) ?></td>
                    <td><?= htmlspecialchars($row['reference_number'] ?? 'N/A') ?></td>
                    <td>
                        <a href="#" class="customer-link" 
                            data-bs-toggle="modal" 
                            data-bs-target="#orderDetailsModal"
                            data-order-id="<?= htmlspecialchars($displayOrderId) ?>"
                            data-customer-name="<?= htmlspecialchars($row['customer_name'] ?? 'Unknown') ?>"
                            data-email="<?= htmlspecialchars($row['email'] ?? 'N/A') ?>"
                            data-contact="<?= htmlspecialchars($row['contact'] ?? 'N/A') ?>"
                            data-address="<?= htmlspecialchars($row['address'] ?? 'N/A') ?>"
                            data-order-type="<?= htmlspecialchars($row['order_type'] ?? '-') ?>"
                            data-quantity="<?= htmlspecialchars($row['quantity'] ?? '1') ?>"
                            data-total-price="₱<?= number_format($row['total_price'], 2) ?>"
                            data-downpayment-amount="<?= $row['downpayment_amount'] ?? 0 ?>"
                            data-expected-date="<?= htmlspecialchars($displayDate) ?>" 
                            data-design-file-b64="<?= base64_encode($row['design_file'] ?? '') ?>"
                            data-order-details='<?= htmlspecialchars($row['order_details'] ?? '{}', ENT_QUOTES, 'UTF-8') ?>'>
                            <?= htmlspecialchars($row['customer_name'] ?? 'Unknown') ?>
                        </a>
                    </td>
                    <td><?= htmlspecialchars($row['order_type'] ?? '-') ?></td>
                    <td><span class="fw-semibold text-info"><?= $displayDate ?></span></td>
                    <td>₱<?= number_format($row['total_price'], 2) ?></td>
                    <td>
                        <?php if(!empty($imgUrl)): ?>
                            <img src="<?= htmlspecialchars($imgUrl) ?>" 
                                alt="Proof of Payment" 
                                class="payment-proof"
                                data-bs-toggle="modal"
                                data-bs-target="#proofModal"
                                data-img="<?= htmlspecialchars($imgUrl) ?>">
                        <?php elseif(!empty($downpaymentImgUrl)): ?>
                            <a href="#" class="cash-downpayment-link"
                                data-bs-toggle="modal"
                                data-bs-target="#downpaymentModal"
                                data-img="<?= htmlspecialchars($downpaymentImgUrl) ?>"
                                data-total="<?= number_format($row['total_price'], 2) ?>"
                                data-downpayment="<?= number_format($row['downpayment_amount'], 2) ?>"
                                data-remaining="<?= number_format($row['total_price'] - $row['downpayment_amount'], 2) ?>"
                                data-reference="<?= htmlspecialchars($row['downpayment_reference'] ?? 'N/A') ?>">
                                CASH
                            </a>
                        <?php else: ?>
                            <span class="text-success fw-semibold">Cash</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <span class="badge 
                            <?= ($row['status']=='Pending'?'bg-warning':($row['status']=='Processing'?'bg-info':($row['status']=='Completed'?'bg-success':($row['status']=='Received'?'bg-danger':'bg-danger')))) ?>">
                            <?= htmlspecialchars($row['status']) ?>
                        </span>
                        </td>
                    
                    <td>
                        <form method="POST" class="d-flex justify-content-center align-items-center">
                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>">
                            
                            <?php 
                                $currentStatus = trim($row['status']);
                                $isFinalState = ($currentStatus == 'Completed' || $currentStatus == 'Received' || $currentStatus == 'Cancelled');
                            ?>

                            <select name="status" class="form-select form-select-sm status-select me-2" <?= $isFinalState ? 'disabled' : '' ?>>
                                <?php if ($currentStatus == 'Pending'): ?>
                                    <option value="Pending" selected>Pending</option>
                                    <option value="Processing">Processing</option>
                                    <option value="Completed">Completed</option>
                                <?php elseif ($currentStatus == 'Processing'): ?>
                                    <option value="Processing" selected>Processing</option>
                                    <option value="Completed">Completed</option>
                                <?php elseif ($currentStatus == 'Completed'): ?>
                                    <option value="Completed" selected>Completed</option>
                                <?php elseif ($currentStatus == 'Cancelled'): ?>
                                    <option value="Cancelled" selected>Cancelled</option>
                                <?php elseif ($currentStatus == 'Received'): ?>
                                    <option value="Received" selected>Received</option>
                                <?php else: ?>
                                    <option value="<?= htmlspecialchars($currentStatus) ?>" selected><?= htmlspecialchars($currentStatus) ?></option>
                                    <option value="Pending">Pending</option>
                                    <option value="Processing">Processing</option>
                                    <option value="Completed">Completed</option>
                                    <option value="Received">Received</option>
                                <?php endif; ?>
                            </select>
                            
                            <button type="submit" name="update_status" class="btn btn-sm btn-success" <?= $isFinalState ? 'disabled' : '' ?>>Update</button>
                        </form>
                    </td>

                </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="9" class="text-muted">No orders found with the current search and filter criteria.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
</div>

<div class="modal fade" id="proofModal" tabindex="-1" aria-labelledby="proofModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-body text-center">
        <img src="" id="proofImage" class="modal-img" alt="Payment Proof">
      </div>
    </div>
  </div>
</div>

<div class="modal fade" id="downpaymentModal" tabindex="-1" aria-labelledby="downpaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="downpaymentModalLabel">Downpayment Details</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <h6><strong>Payment Breakdown</strong></h6>
            <table class="payment-breakdown-table">
              <tr>
                <td>Total Amount:</td>
                <td id="modalTotalAmount">₱0.00</td>
              </tr>
              <tr>
                <td>Downpayment (75%):</td>
                <td id="modalDownpaymentAmount">₱0.00</td>
              </tr>
              <tr>
                <td>Remaining Amount:</td>
                <td id="modalRemainingAmount">₱0.00</td>
              </tr>
            </table>
            <p class="mt-3"><strong>Reference Number:</strong> <span id="modalDownpaymentReference">N/A</span></p>
          </div>
          <div class="col-md-6">
            <h6><strong>Proof of Payment</strong></h6>
            <img src="" id="downpaymentProofImage" class="modal-img" alt="Downpayment Proof">
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>

<div class="modal fade" id="orderDetailsModal" tabindex="-1" aria-labelledby="orderDetailsModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="orderDetailsModalLabel">Order <span id="modalOrderIdHeader" class="fw-bold"></span></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <h6><strong>Booking Details</strong></h6>
            <p><strong>Order ID:</strong> <span id="modalOrderId"></span></p>
            <p><strong>Customer Name:</strong> <span id="modalCustomerNameBody"></span></p>
            <p><strong>Email:</strong> <span id="modalEmail"></span></p>
            <p><strong>Contact:</strong> <span id="modalContact"></span></p>
            <p><strong>Notes:</strong> <span id="modalAddress"></span></p>
          </div>
          <div class="col-md-6">
            <h6><strong>Service Details</strong></h6>
            <p><strong>Service Needed:</strong> <span id="modalOrderType"></span></p>
            <div id="modalServiceSpecifics" class="ps-3 mb-2"></div>
            <p><strong>Quantity:</strong> <span id="modalQuantity"></span></p>
            
            <div id="modalPaymentBreakdown" style="display: none;">
              <table class="payment-breakdown-table">
                <tr>
                  <td>Total Amount:</td>
                  <td id="modalDetailTotalPrice">₱0.00</td>
                </tr>
                <tr>
                  <td>Downpayment:</td>
                  <td id="modalDetailDownpayment">₱0.00</td>
                </tr>
                <tr>
                  <td>Remaining Balance:</td>
                  <td id="modalDetailRemaining">₱0.00</td>
                </tr>
              </table>
            </div>
            
            <p id="modalSimpleTotalPrice"><strong>Total Price:</strong> <span id="modalTotalPriceValue"></span></p>
            
            <p><strong>Expected Date:</strong> <span id="modalExpectedDate"></span></p>
            <hr>
            <div class="design-file-header">
                <h6><strong>Design File</strong></h6>
                <div id="downloadAllBtnContainer"></div>
            </div>
            <div id="modalDesignFileContainer"></div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/FileSaver.js/2.0.0/FileSaver.min.js"></script>

<script>
document.getElementById('sidebarToggle').addEventListener('click', function() {
    document.getElementById('sidebar').classList.toggle('active');
    document.getElementById('overlay').classList.toggle('active');
});

document.getElementById('overlay').addEventListener('click', function() {
    document.getElementById('sidebar').classList.remove('active');
    document.getElementById('overlay').classList.remove('active');
});

function updateDateTime() {
    const now = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true };
    const dateTimeString = now.toLocaleDateString('en-US', options).replace(',', '') + ' | ' + now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
    document.getElementById('currentDateTime').textContent = dateTimeString;
}
updateDateTime();
setInterval(updateDateTime, 1000);

const proofModal = document.getElementById('proofModal');
if (proofModal) {
    proofModal.addEventListener('show.bs.modal', event => {
        const button = event.relatedTarget;
        const imgSrc = button.getAttribute('data-img');
        document.getElementById('proofImage').src = imgSrc;
    });
}

const downpaymentModal = document.getElementById('downpaymentModal');
if (downpaymentModal) {
    downpaymentModal.addEventListener('show.bs.modal', event => {
        const button = event.relatedTarget;
        
        const imgSrc = button.getAttribute('data-img');
        const total = button.getAttribute('data-total');
        const downpayment = button.getAttribute('data-downpayment');
        const remaining = button.getAttribute('data-remaining');
        const reference = button.getAttribute('data-reference');
        
        document.getElementById('downpaymentProofImage').src = imgSrc;
        // Safe fallback for elements
        if(document.getElementById('modalTotalAmount')) document.getElementById('modalTotalAmount').textContent = '₱' + total;
        if(document.getElementById('modalDownpaymentAmount')) document.getElementById('modalDownpaymentAmount').textContent = '₱' + downpayment;
        if(document.getElementById('modalRemainingAmount')) document.getElementById('modalRemainingAmount').textContent = '₱' + remaining;
        if(document.getElementById('modalDownpaymentReference')) document.getElementById('modalDownpaymentReference').textContent = reference;
    });
}

const orderDetailsModal = document.getElementById('orderDetailsModal');
if (orderDetailsModal) {
    orderDetailsModal.addEventListener('show.bs.modal', event => {
        const button = event.relatedTarget;

        const orderId = button.getAttribute('data-order-id');
        const customerName = button.getAttribute('data-customer-name');
        const email = button.getAttribute('data-email');
        const contact = button.getAttribute('data-contact');
        const address = button.getAttribute('data-address');
        const orderType = button.getAttribute('data-order-type');
        const quantity = button.getAttribute('data-quantity');
        const totalPrice = button.getAttribute('data-total-price');
        const downpaymentAmount = parseFloat(button.getAttribute('data-downpayment-amount')) || 0;
        const expectedDate = button.getAttribute('data-expected-date');
        const orderDetailsJson = button.getAttribute('data-order-details');
        
        const designFileB64 = button.getAttribute('data-design-file-b64');
        let designFile = '';

        if (designFileB64) {
            try {
                designFile = atob(designFileB64);
            } catch (e) {
                console.error("Base64 decoding failed:", e);
                designFile = '';
            }
        }
        
        // HELPER FUNCTION TO PREVENT CRASHES
        const safeSetText = (selector, text) => {
            const el = orderDetailsModal.querySelector(selector);
            if (el) {
                el.textContent = text;
            } else {
                console.warn(`Element not found: ${selector}`);
            }
        };

        // Set Values Safely
        safeSetText('#modalOrderIdHeader', '#' + orderId);
        safeSetText('#modalOrderId', orderId);
        safeSetText('#modalCustomerNameBody', customerName);
        safeSetText('#modalEmail', email);
        safeSetText('#modalContact', contact);
        safeSetText('#modalAddress', address);
        safeSetText('#modalOrderType', orderType);
        safeSetText('#modalQuantity', quantity);
        safeSetText('#modalExpectedDate', expectedDate);
        
        const paymentBreakdownDiv = orderDetailsModal.querySelector('#modalPaymentBreakdown');
        const simpleTotalPriceP = orderDetailsModal.querySelector('#modalSimpleTotalPrice');
        
        if (downpaymentAmount > 0) {
            if(paymentBreakdownDiv) paymentBreakdownDiv.style.display = 'block';
            if(simpleTotalPriceP) simpleTotalPriceP.style.display = 'none';
            
            // Global replacement for commas to handle thousands (e.g., 1,500.00)
            const totalValue = parseFloat(totalPrice.replace('₱', '').replace(/,/g, ''));
            const remaining = totalValue - downpaymentAmount;
            
            safeSetText('#modalDetailTotalPrice', totalPrice);
            safeSetText('#modalDetailDownpayment', '₱' + downpaymentAmount.toFixed(2));
            safeSetText('#modalDetailRemaining', '₱' + remaining.toFixed(2));
        } else {
            if(paymentBreakdownDiv) paymentBreakdownDiv.style.display = 'none';
            if(simpleTotalPriceP) simpleTotalPriceP.style.display = 'block';
            safeSetText('#modalTotalPriceValue', totalPrice);
        }
        
        const specificsContainer = orderDetailsModal.querySelector('#modalServiceSpecifics');
        if (specificsContainer) {
            specificsContainer.innerHTML = '';
            try {
                const details = JSON.parse(orderDetailsJson);
                for (const [key, value] of Object.entries(details)) {
                    if (value) {
                        const p = document.createElement('p');
                        p.style.margin = '0';
                        p.style.fontSize = '0.9rem';
                        p.innerHTML = `<strong>${key}:</strong> ${value}`;
                        specificsContainer.appendChild(p);
                    }
                }
            } catch (e) {
                console.error("Could not parse order details JSON:", e);
                specificsContainer.innerHTML = '<p class="text-muted">No specific details available.</p>';
            }
        }

        const modalDesignFileContainer = orderDetailsModal.querySelector('#modalDesignFileContainer');
        const downloadAllContainer = orderDetailsModal.querySelector('#downloadAllBtnContainer');
        
        if (modalDesignFileContainer) modalDesignFileContainer.innerHTML = ''; 
        if (downloadAllContainer) downloadAllContainer.innerHTML = '';

        if (designFile) {
            const files = designFile.split(',').map(f => f.trim()).filter(f => f.length > 0);
            const fileLinks = []; 

            files.forEach((cleanedFile, index) => {
                if (cleanedFile) {
                    const fileUrl = '../uploads/designs/' + cleanedFile;
                    fileLinks.push({ url: fileUrl, name: cleanedFile }); 

                    const downloadLink = document.createElement('a');
                    downloadLink.href = fileUrl;
                    downloadLink.textContent = `Design File ${index + 1}`;
                    downloadLink.className = 'btn btn-primary btn-sm me-1 mb-1';
                    downloadLink.setAttribute('download', cleanedFile);
                    if(modalDesignFileContainer) modalDesignFileContainer.appendChild(downloadLink);
                }
            });

            if (files.length > 1 && downloadAllContainer) {
                const downloadAllBtn = document.createElement('button');
                downloadAllBtn.textContent = 'Download All (.zip)';
                downloadAllBtn.className = 'btn btn-outline-success btn-sm download-all-btn';
                
                downloadAllBtn.onclick = async () => {
                    downloadAllBtn.textContent = 'Zipping...';
                    downloadAllBtn.disabled = true;
                    
                    try {
                        const zip = new JSZip();
                        const filePromises = fileLinks.map(file => 
                            fetch(file.url)
                                .then(response => {
                                    if (!response.ok) {
                                        throw new Error(`Failed to fetch ${file.name}`);
                                    }
                                    return response.blob();
                                })
                                .then(blob => {
                                    zip.file(file.name, blob);
                                })
                        );
                        await Promise.all(filePromises);
                        zip.generateAsync({ type: "blob" })
                            .then(content => {
                                saveAs(content, `Order_${orderId}_Designs.zip`);
                                downloadAllBtn.textContent = 'Download All (.zip)';
                                downloadAllBtn.disabled = false;
                            });

                    } catch (error) {
                        console.error('Error creating zip file:', error);
                        downloadAllBtn.textContent = 'Error';
                        setTimeout(() => {
                             downloadAllBtn.textContent = 'Download All (.zip)';
                                 downloadAllBtn.disabled = false;
                        }, 2000);
                    }
                };
                downloadAllContainer.appendChild(downloadAllBtn);
            }
            
        } else {
            if(modalDesignFileContainer) modalDesignFileContainer.innerHTML = '<p class="text-muted">No design file was uploaded for this order.</p>';
        }
    });
}
</script>

</body>
</html>

<?php $conn->close(); ?>